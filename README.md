...
**Simulateur TNS / SASU (IR / IS) pour la France**  
À partir du chiffre d’affaires, calcule rémunération, cotisations, impôts, net foyer et projection pluriannuelle.  
Version : v1.3 (front-end statique, JavaScript/HTML/CSS, côté client).
...
Ce projet aide un dirigeant (TNS, SASU à l’IR, SASU à l’IS, ou micro-entreprise) à simuler, à partir d’un chiffre d’affaires :
- Sa rémunération effective (brute/net), ses cotisations, impôts (IR, IS, PFU/barème), dividendes, etc.
...
- Ligne par année avec détail : revenus, impôts, net, cotisations, warning micro si applicable.

### Synchronisation et agrégation
- Synchronisation automatique de la source principale vers le calcul de l’IR du foyer.  
...
- Référence dans les notes expliquant la règle (avec lien vers sources officielles à intégrer).
...
* `script.js` : logique complète de simulation (IR, IS, TNS, SASU, micro-entreprise, projection, export, UI state, synchronisation).
* `styles.css` : thème, layout responsive, warnings, mise en forme des tableaux et KPI.
...
* `projectYears()` : projection pluriannuelle, inclut logique de blocage micro.
* `computeTaxFromBareme()` : cœur du calcul de l’IR progressif.
* `syncIR()` : synchronisation de la source vers les cases IR.
...
* Pour toute contribution destinée à être re-licenciée ou utilisée commercialement, **un accord explicite** devra être clarifié (ex : Contributor License Agreement).
...
* Extraire les fonctions clés (`computeTaxFromBareme`, `tnsCotisations`, `calcSISU`, `calcMICRO`) dans un module testable.
* Ajouter des cas de référence connus (ex : CA seuil micro, dépassements successifs).
...
1. Choisis “Micro-entreprise”, définis `CA` et “Type d’activité”.
2. Clique sur “Calculer (Micro-entreprise année 1)”.
3. Observe le KPI et le warning si seuil dépassé.
4. Lance “Calculer la projection” : vérifie les éventuels dépassements consécutifs dans les années suivantes.
