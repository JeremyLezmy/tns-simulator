...
function getCommonOptions(colors, title) {
  return {
    ...
    events: ["mousemove", "mouseout", "click", "touchstart", "touchmove", "touchend", "pointerdown", "pointermove", "pointerup"],
    interaction: {
      mode: "nearest",
      axis: "xy",
      ...
      datasets: [
        {
          ...
          backgroundColor: [colors.revenue, colors.social, colors.expense, colors.net],
        },
      ],
    },
    options: {
      ...
      datasets: [
        {
          ...
          backgroundColor: [colors.grid, colors.social, colors.tax, "#f43f5e", "transparent"],
          stack: "stack1",
        },
        {
          label: "Salaire Net",
...
function getIrChartConfig(data) {
  ...
  return {
    ...
    // Lightweight, mobile-safe labels (k€) drawn manually
    plugins: [
      {
        id: "irValueLabels",
        afterDatasetsDraw: (chart) => {
          const { ctx } = chart;
          ctx.save();
          ctx.fillStyle = colors.highContrastText;
          ctx.textAlign = "center";
          ctx.textBaseline = "middle";
          ...
          chart.data.datasets.forEach((dataset, dIndex) => {
            ...
            dataset.data.forEach((raw, i) => {
              ...
              let value = 0;
              if (Array.isArray(raw)) value = raw[1] - raw[0];
              else value = raw;
              if (!value || Math.abs(value) < 1000) return;

              const el = meta.data[i];
              if (!el) return;
              // Center label inside the bar/segment
